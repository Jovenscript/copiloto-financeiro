...
// Status de um Orçamento no mês: quanto já foi gasto (lançamentos apontando
// pra ele via orcamentoId), quanto resta do limite mensal.
export function statusOrcamento(orcamento, lancs = [], ym = chaveMes()) {
  const gasto = lancs
    .filter((l) => l.tipo === 'despesa' && l.orcamentoId === orcamento.id && (l.data || '').startsWith(ym))
    .reduce((s, l) => s + (Number(l.valor) || 0), 0);
  const limite = Number(orcamento.valorMensal) || 0;
  const restante = limite - gasto;
  const pct = limite > 0 ? Math.min(100, (gasto / limite) * 100) : 0;
  return { gasto, limite, restante, pct, estourou: gasto > limite };
}
