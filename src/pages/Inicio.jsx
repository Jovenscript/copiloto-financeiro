...
import { chaveMes, panoramaMes, vencimentosProximos, contasVencidas, statusEnvelope, gastosPorCategoria, mesesAte } from '../core/calculos';
...
// próximos pagamentos → envelopes (carrossel) → rosca de categorias → meta.
        ...
        {envelopes.length > 0 && (
          <section>
            <div className="flex items-baseline justify-between px-0.5 mb-2.5">
              <h3 className="font-bold text-base text-cream">Envelopes do mês</h3>
            ...
            <div className="flex gap-2.5 overflow-x-auto no-scrollbar pb-1 lg:grid lg:grid-cols-3">
              {envelopes.map((e) => {
                ...
                return (
                  <div key={e.id} className="min-w-[148px] bg-surface shadow-card rounded-2xl p-3.5 shrink-0">
                    <div className="flex items-center gap-2 mb-2.5">
                      <span className="w-[30px] h-[30px] rounded-[9px] bg-accent/8 grid place-items-center text-sm">{info.icone}</span>
                      <span className="text-[13px] font-semibold text-cream">{info.label}</span>
                    </div>
                    <p className="font-num text-[19px] text-cream">{formatarBRL(Math.max(0, s.restante))}</p>
                    <p className="text-[11px] text-muted mb-2">de {formatarBRL(s.meta)}</p>
